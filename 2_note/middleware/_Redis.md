...
- [一、基础部分](#基础部分)
    - [01、Redis 五大数据类型](#Redis-五大数据类型)
    - [02、Redis 五大数据类型的使用场景](#Redis-五大数据类型的使用场景)
    - [03、ZSet 的使用场景](#ZSet-的使用场景)
    - [04、跳表是什么](#跳表是什么)
    - [05、Redis 为什么用跳表不用树](#Redis-为什么用跳表不用树)
    - [06、压缩表是什么](#压缩表是什么)
- [二、提高部分](#提高部分)
    - [01、Redis 缓存雪崩](#Redis-缓存雪崩)
    - [02、Redis 缓存穿透](#Redis-缓存穿透)
    - [03、Redis 缓存击穿](#Redis-缓存击穿)
    - [04、Redis 实现分布式锁的缺点](#Redis-实现分布式锁的缺点)
    - [05、Redis 和 Zookeeper 的分布式锁区别](#Redis-和-Zookeeper-的分布式锁区别)
    - [06、Redis 为什么这么快](#Redis-为什么这么快)
    - [07、Redis 怎么实现消息队列](#Redis-怎么实现消息队列)
    - [08、Redis 主从复制原理](#Redis-主从复制原理)
    - [09、Redis 过期键的清理策略](#Redis-过期键的清理策略)
    - [10、Redis 和 MySQL 的双写一致性](#Redis-和-MySQL-的双写一致性)
    - [11、Redis 事务和 ACID 属性](#Redis-事务和-ACID-属性)
    - [12、Redis 是单线程还是多线程](#Redis-是单线程还是多线程)
    - [13、为什么 Redis 在 4.0 之前会选择使用单线程](#为什么-Redis-在-4.0-之前会选择使用单线程)
    - [14、为什么 Redis 使用单线程还那么快](#为什么-Redis-使用单线程还那么快)
    - [15、Redis 是如何实现数据不丢失的](#Redis-是如何实现数据不丢失的)
    - [16、AOF 和 RDB 的实现原理](#AOF-和-RDB-的实现原理)
    - [17、Redis 为什么要先执行命令，再把数据写入日志](#Redis-为什么要先执行命令，再把数据写入日志)
    - [18、Redis 是单线程还是多线程](#Redis-是单线程还是多线程)
    - [19、后写日志有什么风险](#后写日志有什么风险)
    - [20、RDB 做快照时会阻塞线程吗](#RDB-做快照时会阻塞线程吗)
    - [21、RDB 做快照的时候数据能修改吗](#RDB-做快照的时候数据能修改吗)
    - [22、Redis 怎么解决在 bgsave 做快照的时候允许数据修改的](#Redis-怎么解决在-bgsave-做快照的时候允许数据修改的)
    - [23、Redis 如何实现高可用](#Redis-如何实现高可用)
    - [24、为什么需要使用集群模式](#为什么需要使用集群模式)
- [三、底层原理](#底层原理)
    - [01、redisObject 内部结构](#redisObject-内部结构)
    - [02、redisObject 如何表示 string](#redisObject-如何表示-string)
    - [03、redisObject 如何表示 list](#redisObject-如何表示-list)
    - [04、redisObject 如何表示 hash](#redisObject-如何表示-hash)
    - [05、redisObject 如何表示 set](#redisObject-如何表示-set)
    - [06、redisObject 如何表示 zset](#redisObject-如何表示-zset)
...
2、主动删除。定时清理，每次清理都会遍历 DB。

3、内存不够时清理。当使用内存超过了设置的最大内存，就会释放内存，按照配置的淘汰策略清理内存。
