...
1. Record_Lock ：锁记录。
2. Gap_Lock ：锁记录前的 GAP。
3. Next-Key Lock ：同时锁记录 + 记录前的 GAP。
4. insert_Intention_Lock ：插入意向锁，其实是特殊的 GAP 锁。
...
1. 线性探查：发生 hash 冲突时，顺序查找下一个位置，直到找到一个空位置（固定步长 1 探测）。
2. 二次探查：在发生 hash 冲突时，在表的左右位置进行按一定步长跳跃式探测（固定步长 n 探测）。
3. 伪随机探测：在发生 hash 冲突时，根据公式生成一个随机数，作为此次探测空位置的步长（随机步长 n 探测）。
