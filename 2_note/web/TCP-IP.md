...
### 为什么 TIME_WAIT 要等待 2MSL / 2MSL 的作用
...
视频讲解：[一条视频讲清楚 TCP 协议与 UDP 协议——什么是三次握手与四次挥手](https://www.bilibili.com/video/BV1kV411j7hA) （推荐）
...
客户端 CLOSE ——> 服务端 **LISTEN** ——> 客户端 SYN_SENT ——> 服务端 SYN_RCVD ——> ESTABLISHED

### 四挥的状态变化

客户端 FIN_WAIT-1 ——> 服务端 **CLOSE_WAIT** ——> 客户端 FIN_WAIT-2 ——> 服务端 LAST_ACK ——> 客户端 **TIME_WAIT** ——>
CLOSE

### 滑动窗口 / Sliding Window Protocol

属于 TCP 协议的一种应用，用于网络数据传输时的流量控制，以避免拥塞的发生。
...
2、一个一个增加实在是太慢了，所以可以刚开始发送 1 个，如果没有发生超时时间，就发送 2 个，如果还是没有发送超时事件就发送 4 个，接着 8 个...，用翻倍的速度类推，即 N = 1, 2,
4, 8, 16...

### 如何知道网络的拥塞情况

A 与 B 建立连接之后，就可以向 B 发送数据了，然而这个时候 A 并不知道此时的网络拥塞情况如何，也就是说，A 不知道一次性连续发送多少个数据包好，我们也把 A
一次性连续发送多少个数据包称之为拥塞窗口，用 N 代表此时拥塞窗口的大小吧。

为了探测网络的拥塞情况，我们可以采取以下两种策略：

1、先发送一个数据包试探下，如果该数据包没有发生超时事件 (也就是没有丢包)。那么下次发送时就发送 2 个，如果还是没有发生超时事件，下次就发送 3 个，以此类推，即 N = 1, 2, 3, 4,
5.....
...
最终的策略是：前期指数增长，到达阈值之后，就以一个一个线性的速度来增长。
...
视频讲解：[TCP/IP 网络通信之 Socket 编程入门](https://www.bilibili.com/video/BV1eg411G7pW)
...
- [通俗易懂跟你讲解什么是拥塞控制](https://zhuanlan.zhihu.com/p/97709686)
- []()
- []()
- []()
