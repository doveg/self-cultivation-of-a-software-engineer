...
- [三、索引](#索引)
    - [01、MySQL 为什么需要索引：磁盘的遍历性能太低](#MySQL-为什么需要索引)
    - [02、MySQL 的索引分类：主键、唯一、普通索引、全文索引](#MySQL-的索引分类)
    - [03、联合索引 / 组合索引 / 复合索引：最左前缀原则](#联合索引-/-组合索引-/-复合索引)
    - [04、前缀索引：前部分字符做索引](#前缀索引)
    - [05、覆盖索引：避免回表](#覆盖索引)
    - [06、索引下推：对二级索引有效](#索引下推)
    - [07、聚簇索引、非聚簇索引的区别：数据和主键索引存储是否存在一起](#聚簇索引、非聚簇索引的区别)
    - [08、主键索引、非主键索引的区别：叶子节点存放存放的是否是整行数据](#主键索引、非主键索引的区别)
    - [09、InnoDB 中的 B+Tree 索引](#InnoDB-中的-B+Tree-索引)
    - [10、B 树、B+ 树的区别](#B-树、B+-树的区别)
    - [11、hash 索引、B+ 树索引的区别](#hash-索引、B+-树索引的区别)
    - [12、有那些解决 hash 冲突的方法](#有那些解决-hash-冲突的方法)
    - [13、为什么官方建议使用自增长主键](#为什么官方建议使用自增长主键)
    - [14、索引失效](#索引失效)
    - [15、如何做索引优化](#如何做索引优化)
- [四、事务](#事务)
    - [01、事务的特性 / ACID：独立单元、合法完整、互不干扰、永久改变](#事务的特性-/-ACID)
    - [02、事务的隔离级别：读未提交、读已提交、可重复读、串行化](#事务的隔离级别)
    - [03、幻读 / Phantom Read](#幻读-/-Phantom-Read)
    - [04、MVCC 的快照读和当前读](#MVCC-的快照读和当前读)
    - [05、MySQL 的 InnoDB 如何实现 MVCC：事务的版本号](#MySQL-的-InnoDB-如何实现-MVCC)
    - [06、MySQL 如何实现原子性：undo log](#MySQL-如何实现原子性)
    - [07、MySQL 如何实现持久性：redo log](#MySQL-如何实现持久性)
    - [08、MySQL 如何实现隔离性：锁机制](#MySQL-如何实现隔离性)
    - [09、MySQL 如何实现一致性：数据库层面、应用层面](#MySQL-如何实现一致性)
- [五、日志](#日志)
...
- [六、锁](#锁)
    ...
    - [03、InnoDB 中的行锁：记录锁、间隙锁、下键锁、插入意向锁](#InnoDB-中的行锁)
    - [04、锁组合](#锁组合)
    - [05、悲观锁和乐观锁：读数据加锁、读版本号后更新时再比对](#悲观锁和乐观锁)
- [七、MySQL 架构](#MySQL_架构)
    - [01、锁的作用范围分类](#锁的作用范围分类)
    - [](#)
...
假设先写 redo log 直接提交，然后写 binlog，写完 redo log 后，机器挂了，binlog 日志没有被写入，那么机器重启后，这台机器会通过 redo log 恢复数据，但是这个时候
binlog 并没有记录该数据，后续进行机器备份的时候，就会丢失这一条数据，同时主从同步也会丢失这一条数据。
...
**InnoDB 的 MVCC 是通过 read view 和版本链实现的，版本链保存有历史版本记录，通过 read view
...
快照读情况下，InnoDB 通过 mvcc 机制避免了幻读现象。而 mvcc
...
select * from table where id<6 lock in share mode;--共享锁 select * from table where id<6 for
update;--排他锁
